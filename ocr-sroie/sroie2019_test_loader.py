...
# Paths for SROIE2019 test set (resolve relative to repository root)
ROOT_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
BOX_DIR = os.path.join(ROOT_DIR, 'data', 'SROIE2019', 'test', 'box')
ENTITIES_DIR = os.path.join(ROOT_DIR, 'data', 'SROIE2019', 'test', 'entities')
IMG_DIR = os.path.join(ROOT_DIR, 'data', 'SROIE2019', 'test', 'img')
...
def load_sample(file_id):
    ...
    entities = read_json_with_fallback(ent_path)
    # read boxes/words first
    lines = read_lines_with_fallback(box_path)
    for line in lines:
        if not line.strip():
            continue
...
if __name__ == '__main__':
    ...
    print(f"Loaded {len(samples)} samples from SROIE2019 test set.")
